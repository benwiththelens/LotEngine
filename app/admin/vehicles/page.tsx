...
const STATES = ["NE", "IA", "KS", "MO", "CO", "WY", "SD", "MN", "IL", "WI"];
...
export default function VehicleInventory() {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    public_description: "", is_inventory: true, initiate_recon: true,
  ...
  }, [formData, terminal.id, terminal.isOpen, terminal.activeStep, isOnline]);
  ...
  async function refreshInventoryQuietly() {
    const { data: tenant } = await supabase.from("tenants").select("id").eq("domain", window.location.host).single();
    const { data } = await supabase.from("vehicles").select("*").eq("tenant_id", tenant?.id).order("created_at", { ascending: false });
    if (data) setInventory(data);
  }
  ...
  const openAddTerminal = () => {
    setFormData({ vin: "", plate: "", state: "NE", year: "", make: "", model: "", trim: "", engine: "", drivetrain: "FWD", fuel_type: "Gasoline", ev_range: "", ev_battery: "", condition: "Good", features: [], price: "", mileage: "", status: "available", lot_location: "RECEIVING", public_description: "", is_inventory: true, initiate_recon: true, exterior_color: "", interior_color: "" });
        ...
        <div className="max-w-7xl mx-auto"><div className="bg-white border-4 border-black overflow-hidden shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] text-black">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y-4 divide-zinc-50 text-black">
                {loading ? <tr><td colSpan={5} className="p-20 text-center animate-pulse opacity-20 uppercase font-black text-black">Syncing...</td></tr> : inventory.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-zinc-50 group transition-colors text-black text-black">
                      ...
                      <td className="p-5 text-center text-black text-black text-black"><p className="font-mono font-black text-xl text-brand-primary leading-none text-brand-primary">${Number(v.price || 0).toLocaleString()}</p><p className="font-mono text-[10px] opacity-40 uppercase mt-1">{v.mileage?.toLocaleString() || "---"} MI</p></td>
                      <td className="p-5 text-center text-black text-black text-black text-black"><div className="flex flex-wrap justify-center gap-1 text-black">{audit.length === 0 ? <span className="text-[8px] font-black text-green-600 border px-1 uppercase">Verified</span> : audit.map(f => <span key={f as string} className="text-[8px] font-black bg-brand-primary text-white px-1 uppercase border border-black">{f}</span>)}</div></td>
                ...
                {terminal.activeStep === 1 ? (
                  ...
                  <div className="max-w-2xl mx-auto space-y-16 pb-20 text-black">
                    ...
                    <div><p className="text-xs font-black uppercase text-brand-primary mb-6 border-b-4 border-black pb-2 text-black">03. Condition & Recon</p>
                        <div className="space-y-8 text-black text-black text-black">
                    ...
                    <div className="max-w-xl mx-auto py-20 animate-in zoom-in-95 duration-500 text-center text-black">
                        <div className="w-24 h-24 bg-black text-white rounded-full flex items-center justify-center mx-auto mb-10 shadow-[8px_8px_0px_0px_rgba(227,66,52,1)] text-white text-white"><svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={4} d="M5 13l4 4L19 7" /></svg></div>
                        ...
                        <div className="bg-zinc-50 border-4 border-black p-8 text-left space-y-6 mb-12 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] text-black">
                            ...
                            <div className="flex justify-between border-b pb-2 text-black text-black text-black"><p className="text-[10px] font-black opacity-40 uppercase">Price</p><p className="font-mono font-black text-brand-primary text-brand-primary text-brand-primary">${Number(formData.price).toLocaleString()}</p></div>
                        </div>
                        <div className="flex flex-col gap-4 text-black text-black">
                            ...
                            <button onClick={() => setTerminal({ ...terminal, isOpen: false })} className="w-full bg-white text-black py-6 font-black uppercase border-4 border-black hover:bg-zinc-50 text-black text-black">Close Terminal</button>
                        </div>
                    </div>
                )}
            </div>
